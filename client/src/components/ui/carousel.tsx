...
const Slide = ({ slide, index, current, handleSlideClick }: SlideProps) => {
  ...
  return (
    <div className="[perspective:1200px] [transform-style:preserve-3d]">
      ...
      >
        <div
          className="absolute left-0 top-0 h-full w-full overflow-hidden rounded-[1%] bg-primary transition-all duration-150 ease-out"
          style={{
            transform:
              current === index
        ...
        >
          ...
          />
          {current === index && (
            <div className="absolute inset-0 bg-primary/80 transition-all duration-1000" />
          )}
        </div>
        ...
        >
          ...
          <div className="flex justify-center">
            <Button
              variant={"outline"}
              asChild
              className="mx-auto mt-6 flex h-12 w-fit items-center justify-center rounded-full border border-transparent bg-white px-4 py-2 text-xs text-black shadow-[0px_2px_3px_-1px_rgba(0,0,0,0.1),0px_1px_0px_0px_rgba(25,28,33,0.02),0px_0px_0px_1px_rgba(25,28,33,0.08)] transition duration-200 hover:shadow-lg sm:text-sm"
            >
